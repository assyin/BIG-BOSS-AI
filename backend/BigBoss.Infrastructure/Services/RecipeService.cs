...
public class RecipeService : IRecipeService
{
    ...
    private static RecipeDto MapToDto(Recipe recipe)
    {
        return new RecipeDto
        {
            ...
            IngredientsDarija = string.IsNullOrWhiteSpace(recipe.IngredientsDarijaJson) ? new() : JsonSerializer.Deserialize<List<string>>(recipe.IngredientsDarijaJson) ?? new(),
            StepsDarija = string.IsNullOrWhiteSpace(recipe.StepsDarijaJson) ? new() : JsonSerializer.Deserialize<List<string>>(recipe.StepsDarijaJson) ?? new(),
            ...
            Ingredients = string.IsNullOrWhiteSpace(recipe.IngredientsJson) ? new() : JsonSerializer.Deserialize<List<string>>(recipe.IngredientsJson) ?? new(),
            Steps = string.IsNullOrWhiteSpace(recipe.StepsJson) ? new() : JsonSerializer.Deserialize<List<string>>(recipe.StepsJson) ?? new(),
